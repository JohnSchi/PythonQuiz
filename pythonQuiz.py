...
questionFile = 'programmingQuestions.yml'
...
def run_quiz():
    ...
    buttons = []
    label = Label(root, text="How many questions do you want to answer?", wraplength=500, justify="left")
    label.pack(pady=(0,20))
    answer = IntVar()
    
    for num in numQuestions:
        button = Radiobutton(root, text=num, variable=answer, value=num)
        button.pack(anchor=W)
        buttons.append(button)

    root.wait_variable(answer)
    numQuestionsPerQuiz = answer.get()
    label.destroy()
    for rb in buttons:
        rb.destroy()
    ...
    response = messagebox.askyesno("","You got {} correct out of {} questions.".format(num_correct, num) + "\nDo you want to play again?")
    if response == 1:
...
def check_questions(questions):
    ...
    for question in questions:
        if question["numCorrect"] != 5 and question["dateAsked"] != datetime.date.today():
...
def ask_question(question):
    ...
    question["dateAsked"] = datetime.date.today()
